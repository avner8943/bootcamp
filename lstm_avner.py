...
T_steps = 8 # Number of time steps (length of the sequence) used for training
...
class Parameters:
    def __init__(self):
        ...
        self.W_v = Param('W_v',
                         np.random.randn(1, H_size) * weight_sd)
        self.b_v = Param('b_v',
                         np.zeros((1, 1)))
...
def forward_backward(inputs1, inputs2, targets, h_prev, C_prev):
    global paramters
    ...
    for t in range(T_steps):
        ...
        x = np.array([inputs1[t],inputs2[t]]).reshape((2,1))
        y = targets[t].reshape((1,1))
        
        (z_s[t], f_s[t], i_s[t],
        C_bar_s[t], C_s[t], o_s[t], h_s[t],
        v_s[t], y_s[t]) = \
            forward(x, h_s[t - 1], C_s[t - 1]) # Forward pass
            
        loss += -np.log(y_s[t]) # Loss for at t
        #loss += np.abs(-1*y*np.log(y_s[t]))
        #loss = np.abs(y[0][0]- y_s[t][0][0])
    ...
    try:
        #with DelayedKeyboardInterrupt():
            ...
            loss, g_h_prev, g_C_prev = \
                forward_backward(int_1b_r,int_2b_r, sum_intb_r, g_h_prev, g_C_prev)
